...
//        item = modelMapper.map(itemRepository.save(item), ItemDTO.class);
    ...
    public ResponesPageDTO<ItemDTO> list(String email, RequestPageDTO requestPageDTO){
        ...
        itemList.stream().map(item -> modelMapper.map(item, ItemDTO.class)).collect(Collectors.toList());
...
//                .collect(Collectors.toList());
    ...
    public ItemDTO read(Long item_id){
        ...
        List<ImgDTO> imgDTOList = imgService.read(item_id);
    ...
    public ItemDTO update(ItemDTO itemDTO , MultipartFile[] multipartFile , MultipartFile mainimg , Long[] delino) throws IOException {
        ...
        item.setPrice(itemDTO.getPrice());                      // 가격 수정
        item.setItemNm(itemDTO.getItemNm());                    // 상품명 수정
        item.setItemDetail(itemDTO.getItemDetail());            // 상세정보 수정
        item.setItemSellStatus(itemDTO.getItemSellStatus());    // 판매여부 수정
        item.setStockNumber(itemDTO.getStockNumber());          // 수량 수정

        // 상품 이미지 삭제
        // 배열로 들어온 삭제번호를 이용해서 물리적인 파일삭제 그리고 img DB삭제
